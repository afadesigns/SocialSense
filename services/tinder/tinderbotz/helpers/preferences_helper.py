...
class PreferencesHelper:
    ...
    def set_age_range(self, min, max):
        ...
        # start adjusting the distance slider
        while (
            abs(to_percentage_min - current_percentage_min) > 1
            or abs(to_percentage_max - current_percentage_max) > 1
        ):
            ac = ActionChains(self.browser)

            if current_percentage_min < to_percentage_min:
                ac.click_and_hold(btn_minage).move_by_offset(5, 0).release(
                    btn_minage
                ).perform()
            ...
            if current_percentage_max < to_percentage_max:
                ac.click_and_hold(btn_maxage).move_by_offset(5, 0).release(
                    btn_maxage
                ).perform()
        ...
        print(
            "Ended slider with ages from {} years old  to {} years old\n\n".format(
                (current_percentage_min / percentage_per_year) + min_age_tinder,
                (current_percentage_max / percentage_per_year) + min_age_tinder,
            )
        )
