...
class MatchHelper:
    ...
    def _scroll_down(self, xpath):
        ...
        while True:
            ...
            self.browser.execute_script(
                "arguments[0].scrollTo(0, arguments[0].scrollHeight);", eula
            )
    ...
    def send_song(self, chatid, songname):
        ...
        try:
            ...
            song = self.browser.find_element(
                By.XPATH,
    ...
    def unmatch(self, chatid):
        ...
        try:
            #'//button[text()="Unmatch"]'
            unmatch_button = self.browser.find_element(
                By.XPATH,
            ...
            unmatch_button = self.browser.find_element(
                By.XPATH, f"{modal_manager}/div/div/div[2]/button[1]"
            )
            unmatch_button.click()
            time.sleep(1)
    ...
    def _open_chat(self, chatid):
        if self._is_chat_opened(chatid):
            return
        ...
        try:
            match_button = self.browser.find_element(
                By.XPATH, '//a[@href="{}"]'.format(href)
            )
            ...
            try:
                matched_button = self.browser.find_element(
                    By.XPATH, '//a[@href="{}"]'.format(href)
                )
    ...
    def get_image_urls(self, chatid, quickload):
        try:
            ...
            elements = self.browser.find_elements(
                By.XPATH, "//div[@aria-label='Profile slider']"
            )
