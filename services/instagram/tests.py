...
class ClientDirectTestCase(ClientPrivateTestCase):
    def test_direct_thread(self):
        ...
        # send direct photo
        photo = self.cl.direct_send_photo(path="bots/kanada.jpg", user_ids=[instagram])
    ...
    def test_direct_send_photo(self):
        instagram = self.user_id_from_username("instagram")
        dm = self.cl.direct_send_photo(path="bots/kanada.jpg", user_ids=[instagram])
    ...
    def test_upload_video_story(self):
        ...
        try:
            buildout = StoryBuilder(
                path, caption, mentions, Path("bots/background.png")
            ).video(1)
